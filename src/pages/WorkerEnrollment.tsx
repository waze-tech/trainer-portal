  ...
  return (
    <div className="min-h-screen bg-neutral-50 flex flex-col">
            ...
            {(['phone', 'otp', 'info'] as const).map((s, i) => (
              <div key={s} className="flex items-center gap-2 flex-1">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium ${
                  ...
                  (['phone', 'otp', 'info'].indexOf(step) > i) ? 'bg-emerald-500 text-white' :
                  'bg-neutral-200 text-neutral-400'
                }`}>
                  {['phone', 'otp', 'info'].indexOf(step) > i ? <CheckCircle2 size={14} /> : i + 1}
                </div>
                {i < 2 && <div className={`flex-1 h-0.5 ${['phone', 'otp', 'info'].indexOf(step) > i ? 'bg-emerald-500' : 'bg-neutral-200'}`} />}
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex-1 p-4 max-w-md mx-auto w-full">
        ...
        {step === 'phone' && (
          <div className="bg-white rounded-2xl shadow-sm p-6 mt-4">
            <h2 className="text-lg font-semibold text-neutral-900 mb-1">Enter your phone number</h2>
            ...
            <div className="mb-6">
              ...
              <input
                ...
                className="w-full px-4 py-3 text-base border border-neutral-300 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
              />
            </div>
            <button
              onClick={() => setStep('otp')}
              disabled={!phone}
              className="w-full py-3 bg-primary text-white font-medium rounded-xl hover:bg-primary-dark transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 cursor-pointer"
            >
        ...
        {step === 'otp' && (
          <div className="bg-white rounded-2xl shadow-sm p-6 mt-4">
            ...
            <h2 className="text-lg font-semibold text-neutral-900 mb-1">Verify your number</h2>
            ...
            <div className="flex gap-2 justify-center mb-6">
              {otp.map((digit, i) => (
                <input
                  ...
                  className="w-12 h-14 text-center text-xl font-semibold border border-neutral-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
                />
              ))}
            </div>
            <button
              onClick={() => setStep('info')}
              className="w-full py-3 bg-primary text-white font-medium rounded-xl hover:bg-primary-dark transition-colors flex items-center justify-center gap-2 cursor-pointer"
            >
              Verify <ArrowRight size={18} />
            </button>
            <p className="text-center text-xs text-neutral-400 mt-4">
              Didn't receive a code? <button className="text-primary font-medium cursor-pointer">Resend</button>
            </p>
          </div>
        )}
        ...
        {step === 'info' && (
          <div className="bg-white rounded-2xl shadow-sm p-6 mt-4">
            ...
            <h2 className="text-lg font-semibold text-neutral-900 mb-1">Your information</h2>
                ...
                <div key={q.id}>
                  ...
                  {q.type === 'select' && q.options ? (
                    <select
                      ...
                      className="w-full px-4 py-3 text-base border border-neutral-300 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary appearance-none"
                    >
              ...
              <div className="bg-neutral-50 rounded-xl p-4 border border-neutral-200">
                <label className="flex items-start gap-3 cursor-pointer">
                  <input
                    ...
                    className="mt-0.5 rounded border-neutral-300 accent-primary"
                  />
                  <div>
            ...
            <button
              ...
              className="w-full py-3 mt-6 bg-primary text-white font-medium rounded-xl hover:bg-primary-dark transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 cursor-pointer"
            >
        ...
        {/* Success Step */}
        {step === 'success' && (
          <div className="bg-white rounded-2xl shadow-sm p-8 mt-4 text-center">
            <div className="w-16 h-16 bg-emerald-50 rounded-full flex items-center justify-center mx-auto mb-4">
              <CheckCircle2 size={32} className="text-emerald-600" />
            </div>
            <h2 className="text-xl font-semibold text-neutral-900 mb-2">You're enrolled!</h2>
            ...
            <div className="bg-primary/5 rounded-xl p-4 border border-primary/10">
              <p className="text-sm text-primary font-medium">Training Details</p>
      ...
      <div className="p-4 text-center">
        <p className="text-xs text-neutral-400">Powered by <span className="font-medium text-primary">Breadcrumb</span></p>
      </div>
    </div>
  );
}
